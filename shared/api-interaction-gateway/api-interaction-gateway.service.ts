...
export class ApiInteractionGatewayService {
  ...
  private getRequestOptionArgs(param_options?) {
    ...
    let currentUserData = this.commChannelEncryptor.getDecryptedDataFromSessionStorage(IRemember.sessionStorageItems.currentUserData);
    //JSON.parse(sessionStorage.getItem('current_user'));

    const currentUser: IRemLoginResponseInterface = currentUserData ? JSON.parse(currentUserData) : null;
